...
const schema = z.object({
  ...
  // Favicon is independent of the logo (see FIX-027) -- optional so
  // existing sites without a dedicated favicon don't fail validation;
  // the submit handler falls back to the logo only when this is empty.
...
export function AdminCompanySettingsPage() {
  ...
  useEffect(() => {
    if (settings) {
      reset({
        ...
        // Existing sites have logoIcon == logo (the FIX-027 bug); treat
        // that as "no dedicated favicon set yet" rather than pre-filling
        // it as if it were an intentional choice, so the admin notices
        // the new independent field instead of assuming nothing changed.
        faviconSrc: settings.logoIcon && settings.logoIcon.src !== settings.logo.src ? settings.logoIcon.src : '',
  ...
  const onSubmit = async (values: FormValues) => {
    await updateSettings.mutateAsync({
      ...
      // Falls back to the logo only when no dedicated favicon was
      // uploaded, instead of always mirroring the logo (the FIX-027 bug).
      logoIcon: { src: values.faviconSrc?.trim() ? values.faviconSrc : values.logoSrc, alt: `${values.name} favicon` },
    });
  };
    ...
    <div>
      ...
      <form onSubmit={handleSubmit(onSubmit)} noValidate className="space-y-6 max-w-2xl">
        <AdminCard className="space-y-4">
          ...
          <ImageUploadField
            value={watch('logoSrc')}
            onChange={(src) => setValue('logoSrc', src, { shouldValidate: true })}
            category="company"
          />
        ...
        <AdminCard className="space-y-4">
          ...
          <p className="text-sm text-[var(--color-ink-soft)]">
            Shown in browser tabs and bookmarks. Optional -- if left empty, the logo above is used
            instead, but a square, simplified mark usually reads better at tiny sizes than a full
            logo does. Use a square image, ideally 512×512px or larger, PNG with a transparent
            background.
          </p>
          <ImageUploadField
            value={watch('faviconSrc') ?? ''}
            onChange={(src) => setValue('faviconSrc', src, { shouldValidate: true })}
            category="company"
          />
        </AdminCard>
