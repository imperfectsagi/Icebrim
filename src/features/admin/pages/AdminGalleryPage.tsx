...
const EMPTY_FORM: GalleryFormState = { src: '', alt: '', caption: '', category: '' };
...
export function AdminGalleryPage() {
  ...
  const handleAddSubmit = (e: React.FormEvent) => {
    ...
    setAddError(null);
    if (!addForm.src) {
      setAddError('Upload an image first.');
      return;
    }
    if (!addForm.alt.trim()) {
      setAddError('Alt text is required so the image is accessible and SEO-friendly.');
      return;
    }
    createImage.mutate(
      {
      ...
      },
      {
        ...
        onError: (err) => setAddError(err instanceof Error ? err.message : 'Failed to add image.'),
      },
    );
  };
  ...
  const handleEditSubmit = (e: React.FormEvent) => {
    ...
    setEditError(null);
    if (!editForm.src) {
      setEditError('An image is required.');
      return;
    }
  ...
  return (
    <div>
      <AdminPageHeader
        title="Gallery"
        description="Manage the images shown on the public Gallery page. Use the arrows to reorder."
        action={{ label: 'Add Image', onClick: () => setShowAddForm((v) => !v) }}
      />
      ...
      {showAddForm && (
        <AdminCard className="max-w-xl mb-6">
          <form onSubmit={handleAddSubmit} className="space-y-4">
            ...
            <FormRow label="Alt text" hint="Describes the image for accessibility and SEO.">
              <input
      ...
      {editingImage && (
        <AdminModal title="Edit gallery image" onClose={() => setEditingImage(null)}>
