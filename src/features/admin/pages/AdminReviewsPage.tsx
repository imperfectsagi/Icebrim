      ...
      {editingReview && editForm && (
        <AdminModal title="Edit review" onClose={closeEdit}>
          <form onSubmit={handleEditSubmit} className="space-y-4">
            ...
            <FormRow label="Customer-attached photos">
              {editForm.mediaImages.length > 0 ? (
                <div className="flex flex-wrap gap-2">
                  {editForm.mediaImages.map((src, i) => (
                    <div key={src} className="relative h-16 w-16 rounded-lg overflow-hidden border border-[var(--color-line)]">
                      <img src={src} alt="" className="h-full w-full object-cover" />
                      <button
                        type="button"
                        onClick={() =>
                          setEditForm((f) => (f ? { ...f, mediaImages: f.mediaImages.filter((_, idx) => idx !== i) } : f))
                        }
                ...
                <p className="text-sm text-[var(--color-ink-soft)]">The customer didn't attach any extra photos.</p>
              )}
            </FormRow>
            {editError && (
