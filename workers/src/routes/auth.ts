...
auth.post('/login', async (c) => {
  ...
  for (const cookie of setAuthCookies(accessToken, refreshToken, { secure: secureCookie })) {
...
auth.post('/refresh', async (c) => {
  ...
  c.header('Set-Cookie', setAuthCookies(accessToken, refreshToken, { secure: secureCookie })[0], { append: true });
...
auth.post('/logout', async (c) => {
  ...
  for (const cookie of clearAuthCookies({ secure: secureCookie })) {
    c.header('Set-Cookie', cookie, { append: true });
  }
