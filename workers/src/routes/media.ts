...
type MediaContext = Context<{ Bindings: Env; Variables: Partial<AuthedVariables> }>;
...
// Fixed set of storage categories -- see migration 0003_media_categories.sql
// and 0006_rename_banners_category.sql. Keeping this a closed set (rather
// than accepting arbitrary strings) keeps the R2 key structure predictable
// and prevents the category becoming an uncontrolled path-like input.
//
// NOTE: this used to include 'banners' (R2 path `uploads/banners/...`).
// It was renamed to 'hero-media' because that literal path segment is a
// classic ad/tracker filter-list keyword -- EasyList-style content
// blockers (including the tracker-blocking most browsers' private/
// incognito mode enables by default) block any request URL containing
// "banner"/"banners" outright, regardless of what the file actually is.
// That produced `net::ERR_BLOCKED` specifically in Incognito, which looks
// identical to a real outage from the user's side but isn't one -- it's
// the browser refusing to even send the request. See
// migrations/0006_rename_banners_category.sql for the one-time repair of
// already-uploaded rows, and DEPLOYMENT.md §9 for the full writeup.
const MEDIA_CATEGORIES = ['products', 'hero-media', 'blog', 'company', 'gallery', 'reviews', 'other'] as const;
type MediaCategory = (typeof MEDIA_CATEGORIES)[number];

function resolveCategory(raw: string | File | null | undefined): MediaCategory {
  if (typeof raw === 'string' && (MEDIA_CATEGORIES as readonly string[]).includes(raw)) {
    return raw as MediaCategory;
  }
  return 'other';
}
...
adminMedia.post('/upload', async (c) => {
  ...
  const category = resolveCategory(formData?.get('category') ?? null);
  return handleUpload(c as unknown as MediaContext, file, category, c.get('userId'));
});

/**
 * Public, unauthenticated upload used only for customer review
 * attachments (ReviewForm.tsx). Deliberately separate from
 * adminMedia.post('/upload') above rather than just relaxing requireAuth
 * on that route, so every other admin upload surface (products, blog,
 * hero/banner, gallery, pages) stays behind auth -- only this one
 * narrow, rate-limited, fixed-category path is reachable by anonymous
 * visitors. Same file-content verification (magic bytes / real image
 * decode) and size ceilings as the admin path; the only difference is
 * the auth requirement and the IP rate limit below.
 */
export const publicMedia = new Hono<{ Bindings: Env }>();

publicMedia.post('/review-upload', async (c) => {
  const ip = getClientIp(c.req.raw.headers);
  const rateLimitResult = await c.env.FORM_RATE_LIMITER.limit({ key: `review-media:${ip}` });
  ...
  const formData = await c.req.formData().catch(() => null);
  const file = formData?.get('file');
  // Fixed category, not client-supplied -- review attachments always land
  // under uploads/reviews/..., regardless of what the request claims.
  return handleUpload(c as unknown as MediaContext, file, 'reviews', null);
});

async function handleUpload(c: MediaContext, file: File | string | null | undefined, category: MediaCategory, uploaderId: string | null) {
  ...
  if (kind === 'image') {
    return handleImageUpload(c, file, arrayBuffer, category, uploaderId);
  }
  return handleVideoOrGifUpload(c, file, arrayBuffer, kind, category, uploaderId);
}

async function handleImageUpload(c: MediaContext, file: File, arrayBuffer: ArrayBuffer, category: MediaCategory, uploaderId: string | null) {
  ...
  const r2Key = `uploads/${category}/${id}.webp`;
  ...
  await c.env.DB.prepare(
    `INSERT INTO media (id, r2_key, url, filename, content_type, size_bytes, width, height, uploaded_by, media_type, category)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'image', ?)`,
  )
    .bind(
      `media_${id}`,
  ...
  await logAuditEvent(c.env.DB, {
    ...
    metadata: { mediaId: id, sizeBytes: outputBytes.byteLength, mediaType: 'image', category },
  });

  return c.json({ id: `media_${id}`, url: publicUrl, mediaType: 'image' as const, category }, 201);
}

async function handleVideoOrGifUpload(
  c: MediaContext,
  file: File,
  ...
  const id = crypto.randomUUID();
  const r2Key = `uploads/${category}/${id}.${ext}`;
  ...
  await c.env.DB.prepare(
    ...
    .bind(`media_${id}`, r2Key, publicUrl, file.name.slice(0, 200), contentType, arrayBuffer.byteLength, uploaderId, kind, category)
    .run();
  ...
  await logAuditEvent(c.env.DB, {
    ...
    ip: getClientIp(c.req.raw.headers),
    userAgent: c.req.header('User-Agent') ?? null,
    metadata: { mediaId: id, sizeBytes: arrayBuffer.byteLength, mediaType: kind, category },
  });

  return c.json({ id: `media_${id}`, url: publicUrl, mediaType: kind, category }, 201);
}

adminMedia.get('/', async (c) => {
  const categoryFilter = c.req.query('category');
  const isValidCategory = categoryFilter && (MEDIA_CATEGORIES as readonly string[]).includes(categoryFilter);

  const { results } = isValidCategory
    ? await c.env.DB.prepare('SELECT * FROM media WHERE category = ? ORDER BY created_at DESC LIMIT 200')
        .bind(categoryFilter)
        .all()
    : await c.env.DB.prepare('SELECT * FROM media ORDER BY created_at DESC LIMIT 200').all();
