...
adminProducts.post('/', async (c) => {
  ...
  await c.env.DB.prepare(
    `INSERT INTO products (id, slug, name, category, sku, price, offer_price, currency, stock,
       short_description, description, specs, seo_title, seo_description, published, rating_average, rating_count, video_url)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
  )
    .bind(
...
adminProducts.put('/:id', async (c) => {
  ...
  if (input.videoUrl !== undefined) fieldMap.video_url = input.videoUrl || null;
