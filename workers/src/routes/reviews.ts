...
function serializeReview(row: ReviewRow) {
  ...
  try {
    ...
    if (Array.isArray(parsed)) mediaImages = parsed.filter((v): v is string => typeof v === 'string');
  } catch {
    mediaImages = [];
  }
  return {
...
reviews.post('/', async (c) => {
  ...
  await c.env.DB.prepare(
    `INSERT INTO reviews (id, product_slug, author_name, location, rating, title, body, status, ip_address, media_type, media_src, media_images)
     VALUES (?, ?, ?, ?, ?, ?, ?, 'pending', ?, ?, ?, ?)`,
  )
    .bind(
      id,
      ...
      input.mediaSrc ?? null,
      JSON.stringify(input.mediaImages ?? []),
    )
    .run();
