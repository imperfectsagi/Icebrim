...
sitemap.get('/sitemap.xml', async (c) => {
  // PUBLIC_SITE_URL may be a comma-separated list of allowed origins (see
  // lib/cors.ts) -- use only the first (primary/canonical) origin here, or
  // <loc> entries would come out malformed like "https://a.com,https://b.com/products".
  const siteUrl = c.env.PUBLIC_SITE_URL.split(',')[0]!.trim().replace(/\/+$/, '');
